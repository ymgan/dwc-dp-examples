...
ref_event <-
  ...
  dplyr::mutate(
    ...
    minimumDistanceAboveSurfaceInMeters = NA_real_, # subject to discussion: https://github.com/inbo/movepub/issues/109
...
gps_event <-
  ...
  dplyr::mutate(
    ...
    minimumDistanceAboveSurfaceInMeters = dplyr::coalesce(
      .data$`height-above-msl`,
      as.numeric(.data$`height-above-ellipsoid`),
      NA_real_
    ),
    maximumDistanceAboveSurfaceInMeters = dplyr::coalesce(
      .data$`height-above-msl`,
      as.numeric(.data$`height-above-ellipsoid`),
      NA_real_
    ),
    locationRemarks = dplyr::case_when(
      !is.na(.data$`height-above-msl`) ~
        "distance above surface are altitude above mean sea level",
      !is.na(.data$`height-above-ellipsoid`) ~
        "distance above surface are altitude above ellipsoid"
    ),
...
schema_occurrence_assertion$primaryKey <- NULL # Not defined
schema_occurrence_assertion$foreignKeys <- purrr::keep(schema_occurrence_assertion$foreignKeys, ~ .x$fields %in% c("occurrenceID"))
```
