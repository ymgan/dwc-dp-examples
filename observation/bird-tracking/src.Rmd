...
parent_event <-
  ...
  dplyr::mutate(
    ...
    eventDate = format(.data$`deploy-on-date`, format = "%Y-%m-%dT%H:%M:%SZ"),
    # TODO: include deploy-off-date if available as start/end
...
schema_event <- jsonlite::fromJSON("https://raw.githubusercontent.com/gbif/dwc-dp/refs/heads/master/dwc-dp/table-schemas/event.json", simplifyDataFrame = FALSE, simplifyVector = TRUE)
...
schema_occurrence <- jsonlite::fromJSON("https://raw.githubusercontent.com/gbif/dwc-dp/refs/heads/master/dwc-dp/table-schemas/occurrence.json", simplifyDataFrame = FALSE, simplifyVector = TRUE)
...
schema_occurrence_assertion <- jsonlite::fromJSON("https://raw.githubusercontent.com/gbif/dwc-dp/refs/heads/master/dwc-dp/table-schemas/occurrence-assertion.json", simplifyDataFrame = FALSE, simplifyVector = TRUE)

schema_occurrence_assertion$fields <- purrr::keep(schema_occurrence_assertion$fields, ~ .x$name %in% names(occurrenceAssertion))
```
...
```{r}
frictionless::write_package(dwcDP, here::here("observation", "bird-tracking", "output_data"))
```
...
nrow(dwc$emof) == nrow(occurrenceAssertion)
```
